  ...
  await configure({
    sinks: {
      console: (record) => {
        ...
        const coloredLevel = `${
          colors[level as keyof typeof colors] || colors.INFO
        }${colors.BRIGHT}${level.padEnd(5)}${colors.RESET}`;
        ...
        console.log(
          `${colors.DIM}├─${colors.RESET} ${icon} ${coloredLevel} ${colors.DIM}[${coloredCategory}]${colors.RESET}`
        );
        console.log(`${colors.DIM}└─${colors.RESET} ${message}\n`);

        WorkflowLogGateway.getInstance().emitLogRecord({
          level: record.level,
          category: record.category as string | string[],
