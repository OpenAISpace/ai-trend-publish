...
> {
  ...
  async run(
    ...
    try {
      ...
      const items = await step.do(
        ...
        async () => {
          ...
          const LLMProvider = await LLMFactory.getInstance().getLLMProvider(
            await ConfigManager.getInstance().get("AI_SUMMARIZER_LLM_PROVIDER")
          );
          ...
          const enhanceTasks = details.map((item) => async () => {
            try {
              if (item && item.description) {
                ...
                const enhancedDescription =
                  await LLMProvider.createChatCompletion([
                    ...
                    {
                      role: "user",
                      content: `请对以下GitHub项目描述进行润色，使其更加生动、专业且吸引人，保持?00字以内：\n${item.description}`,
                    },
                  ]);

                // 更新项目描述
                logger.debug(
                  `[内容润色] 润色项目: ${item.name} 润色结果: ${enhancedDescription.choices[0]?.message?.content}`
                );
                item.description =
              ...
              logger.warn(`[内容润色] 项目 ${item.name} 润色失败: `, error);
              // 润色失败时保留原始描?
              return item;
            }
          });
      ...
      // 4. 渲染内容
      const { title, htmlContent } = await step.do(
        "generate-content",
        {
          retries: { limit: 2, delay: "5 second", backoff: "exponential" },
          timeout: "5 minutes",
        ...
        async () => {
          ...
          const firstItem = items[0];
          const title = `本期精选 GitHub 热门 AI 开源项目，第一 ${firstItem.name} 项目备受瞩目，发现最新最酷的人工智能开源工具`;
