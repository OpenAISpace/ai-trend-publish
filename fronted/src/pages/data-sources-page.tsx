...
export function DataSourcesPage() {
  ...
  useEffect(() => {
    if (!form.id && (dataSources?.length ?? 0) === 0) {
      setForm({ ...emptyForm });
    }
  }, [dataSources, form.id]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: "数据源已创建", variant: "success" });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: "数据源已更新", variant: "success" });
      queryClient.invalidateQueries({ queryKey: queryKeys.dataSources(session) });
      setForm({ ...emptyForm });
    },
    onError: (error: unknown) => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: "数据源已删除", variant: "success" });
      queryClient.invalidateQueries({ queryKey: queryKeys.dataSources(session) });
      setForm({ ...emptyForm });
    },
    onError: (error: unknown) => {
  ...
  return (
    <>
      <AppHeader
        title="数据源管理"
        subtitle="维护 Firecrawl / Twitter 任务入口，统一驱动采集"
      />
      <AnimatedPage>
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="例如 firecrawl 任务 ID 或 Twitter 列表链接"
                    required
                  />
                </div>
